      ...
      <div className="-mx-4 bg-black px-4 pb-24 pt-4 text-white">
        ...
        <div className="mt-6 flex gap-4 overflow-x-auto pb-2">
          <Link href="/dashboard/staff/profile" className="shrink-0 text-center">
            <span className="mx-auto flex h-20 w-20 items-center justify-center rounded-full border border-fuchsia-400/70 bg-black text-5xl font-semibold text-lime-300 shadow-[0_0_22px_rgba(217,70,239,0.35)]">
              A
            </span>
          ...
          {companies.map((company) => (
            ...
            >
              <span className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[conic-gradient(from_120deg,#bef264,#22d3ee,#fb3d72,#bef264)] p-[4px]">
        ...
        <div className="mt-6 space-y-6">
          {visibleItems.map((item) => {
            ...
            return (
              <article
                key={item.job.id}
                className="relative overflow-hidden rounded-[28px] border border-white/12 bg-[#090d10] shadow-[0_24px_80px_rgba(0,0,0,0.6)]"
              ...
              >
                <div className="flex items-center justify-between gap-3 border-b border-white/10 px-5 py-5">
                  <div className="flex min-w-0 items-center gap-3">
                    <span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full border-2 border-lime-300 bg-black text-xl font-bold text-lime-300">
                      {item.job.organization.name.slice(0, 2).toUpperCase()}
                    </span>
                    <div className="min-w-0">
                      <p className="truncate text-xl font-semibold text-white">{item.job.organization.name}</p>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-lg border border-lime-300 px-4 py-2 text-sm font-semibold text-lime-300"
                  >
                ...
                <div
                  className="relative flex min-h-[360px] flex-col justify-end bg-cover bg-center p-5"
                  style={{
                ...
                >
                  ...
                  <div className="max-w-[88%]">
                    <h3 className="text-5xl font-semibold leading-[0.95] text-white drop-shadow-xl">{item.job.title}</h3>
                ...
                <div className="grid grid-cols-[56px_1fr_56px_92px] items-center gap-3 px-5 py-5">
                    <button
                      type="button"
                      onClick={() => handleSwipe(item, "left")}
                      className={cn(
                        "inline-flex h-14 w-14 items-center justify-center rounded-full border border-white/10 bg-white/[0.03] text-rose-400"
                      )}
                    ...
                    <button
                      ...
                      aria-label="Like job"
                    >
                      <Heart className={cn("h-16 w-16 drop-shadow-[0_0_18px_rgba(251,113,133,0.55)]", item.isLiked ? "fill-current" : "")} />
                      <span className="text-xs text-rose-300">Double tap to like</span>
                    </button>
                    <button
                      type="button"
                      onClick={() => messageCompany(item)}
                      className="inline-flex h-14 w-14 items-center justify-center rounded-full border border-white/10 bg-white/[0.03] text-cyan-300"
                      aria-label="Message company"
                    >
                      <Send className="h-7 w-7" />
                    </button>
                    <Link href={`/jobs/${item.job.id}`} className="block">
                      <Button variant="accent" className="flex h-20 w-20 flex-col gap-1 rounded-full bg-lime-300 p-0 text-center text-black hover:bg-lime-200">
